...
def get_drive_service():
    ...
    Priority:
      1) GOOGLE_SERVICE_ACCOUNT_JSON env (recommended on Railway)
      2) service_account_file path stored in DB (local/termux)
    """
    db = load_db()
...
def find_request_by_id(db: dict, req_id: str):
    return db.get("requests", {}).get(str(req_id))

def find_last_completed_request(db: dict):
...
async def send_slip_pdf(context: ContextTypes.DEFAULT_TYPE, chat_id: int, req: dict, caption: str):
    pdf_path = req.get("slip_pdf_path", "")
    if (not pdf_path) or (not os.path.exists(pdf_path)):
        pdf_path = generate_single_payment_slip_pdf(req)
        req["slip_pdf_path"] = pdf_path
        db = load_db()
        if str(req.get("id")) in db.get("requests", {}):
            db["requests"][str(req["id"])]["slip_pdf_path"] = pdf_path
            save_db(db)
...
async def on_document(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.user_data.get("await_qr_wallet"):
        ...
        await update.message.reply_text(f"✅ QR saved for {label} (as Telegram file).")
        return
    ...
    # save slip path in DB so admin can resend anytime
...
async def on_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    label = db["payment_wallets"][wallet_key]["label"]
    await update.message.reply_text(f"✅ QR saved for {label} (as Telegram photo file).")
