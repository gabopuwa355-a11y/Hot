...
async def on_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if is_admin(user.id):
        ...
        if text.startswith("SET WALLET"):
            ...
            if text in mapping:
                context.user_data["await_wallet"] = mapping[text]
                await update.message.reply_text("Send wallet address now:")
                return

        if context.user_data.get("await_wallet"):
            wallet_key = context.user_data.pop("await_wallet")
            addr = text.strip()
        ...
        if text.startswith("SET QR"):
            ...
            if text in mapping:
                context.user_data["await_qr_wallet"] = mapping[text]
                await update.message.reply_text("Send QR image now.")
                return

        # --- SLIP LAST ---
        if text == "🧾 LAST SLIP (ADMIN)":
            reqs = [r for r in db["requests"].values() if r["status"] == "completed"]
            if not reqs:
                await update.message.reply_text("No completed orders.")
                return
            req = sorted(reqs, key=lambda x: int(x["id"]))[-1]
            await send_slip_pdf(context, ADMIN_ID, req, "Last slip.")
            return
        ...
        if text == "🧾 LAST SLIP (USER+ADMIN)":
            ...
            req = sorted(reqs, key=lambda x: int(x["id"]))[-1]
            await send_slip_pdf(context, ADMIN_ID, req, "Last slip.")
            await send_slip_pdf(context, req["user_id"], req, "Your slip.")
            return

        # --- SLIP BY ID ---
        if text.startswith("🧾 SLIP BY REQUEST"):
            context.user_data["await_req_id"] = True
            await update.message.reply_text("Send request ID.")
            return

        if context.user_data.get("await_req_id"):
            context.user_data.pop("await_req_id")
            rid = text.strip()
            req = db["requests"].get(rid)
            if not req:
                await update.message.reply_text("Request not found.")
                return
            await send_slip_pdf(context, ADMIN_ID, req, "Slip.")
            return
