...
MODE_LABELS = {"disable": "📕 Standard Access", "enable": "📗 Premium Access"}
MODE_EMOJI = {"disable": "📕", "enable": "📗"}
...
DEFAULT_DB = {
    ...
    "payment_qr_file_ids": {"wallet_2": "", "wallet_3": "", "wallet_4": "", "wallet_5": ""},
    "drive": {"file_id": "", "service_account_file": SERVICE_ACCOUNT_FILE},
    "settings": {"admin_notes": "", "quote_validity_minutes": 30},
}

logging.basicConfig(format="%(asctime)s | %(levelname)s | %(name)s | %(message)s", level=logging.INFO)
logger = logging.getLogger("final_single_file_bot")
...
def load_db():
    ...
    for k, v in default.items():
        if k not in db:
            db[k] = v
            changed = True
    for sec in ["payment_wallets", "payment_qr_files", "payment_qr_file_ids"]:
...
def ensure_user(user):
    ...
    if uid not in db["users"]:
        db["users"][uid] = {
            ...
            "joined_at": now, "last_seen_at": now, "orders_count": 0
        }
    else:
...
def item_text(qty):
    return f"{qty} account" if int(qty) == 1 else f"{qty} accounts"

def generate_single_payment_slip_pdf(req):
    ...
    c.drawString(40, y, f"Generated At: {now_text()}")
    y -= 25
    ...
    line(f"Price Per Account: {format_price(req.get('unit_price', 0))}")
    line(f"Total Amount: {format_price(req.get('total_price', 0))}")
    ...
    c.drawString(40, y, f"Delivery Status: {str(req.get('status', '')).upper()}")
    c.save()
    return str(pdf_path)

def find_request_by_id(db, req_id):
    return db.get("requests", {}).get(str(req_id))

def find_last_completed_request(db):
    reqs = [r for r in db.get("requests", {}).values() if r.get("status") == "completed"]
    if not reqs:
        return None
    reqs.sort(key=lambda x: int(x.get("id", 0)), reverse=True)
    return reqs[0]

async def send_slip_pdf(context, chat_id, req, caption):
    db = load_db()
    req = db["requests"].get(str(req["id"]), req)
    pdf_path = req.get("slip_pdf_path", "")
    if (not pdf_path) or (not os.path.exists(pdf_path)):
        pdf_path = generate_single_payment_slip_pdf(req)
        req["slip_pdf_path"] = pdf_path
        db["requests"][str(req["id"])] = req
        save_db(db)
...
def get_drive_service():
    ...
    if SERVICE_ACCOUNT_JSON.strip():
        ...
        credentials = service_account.Credentials.from_service_account_file(sa_file, scopes=["https://www.googleapis.com/auth/drive"])
...
def drive_backup_store_db():
    ...
    created = drive_service.files().create(body={"name": DB_FILE}, media_body=media, fields="id").execute()
    new_id = created["id"]
...
def top_text(prices, selected_mode):
...
def build_main_menu(prices, selected_mode="disable", selected_qty=1):
    ...
    for qty in QTY_LIST:
        ...
        if qty == selected_qty:
            txt = f"✓ {txt} ✓"
        keyboard.append([InlineKeyboardButton(txt, callback_data=f"qty:{qty}")])
...
def request_action_menu(req_id):
    return InlineKeyboardMarkup([
...
def admin_reply_menu():
    keyboard = [
        ...
        [KeyboardButton("SET WALLET 2 (USDT TRC20)"), KeyboardButton("SET WALLET 3 (LTC)")],
        [KeyboardButton("SET WALLET 4 (BNB)"), KeyboardButton("SET WALLET 5 (USDT BEP20)")],
        [KeyboardButton("SET QR 2 (USDT TRC20)"), KeyboardButton("SET QR 3 (LTC)")],
        [KeyboardButton("SET QR 4 (BNB)"), KeyboardButton("SET QR 5 (USDT BEP20)")],
        [KeyboardButton("🧾 LAST SLIP (ADMIN)"), KeyboardButton("🧾 LAST SLIP (USER+ADMIN)")],
        [KeyboardButton("🧾 SLIP BY REQUEST ID (ADMIN)"), KeyboardButton("🧾 SLIP BY REQUEST ID (USER+ADMIN)")],
...
def create_request(db, user, mode, qty):
    req_id = str(db["next_request_id"])
    db["next_request_id"] += 1
    unit_price = float(db["prices"][mode])
    total_price = unit_price * qty
    db["requests"][req_id] = {
        "id": req_id, "user_id": user.id, "username": user.username or "", "name": user.full_name,
        "mode": mode, "qty": qty, "unit_price": unit_price, "total_price": total_price,
        "status": "awaiting_wallet_selection", "wallet_key": "", "wallet_label": "",
        "wallet_address": "", "txid": "", "created_at": now_text(), "updated_at": now_text(),
        "cancel_reason": "", "admin_note": "", "delivered_file_name": "", "slip_pdf_path": "",
    }
    return req_id
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    ensure_user(user)
    state = context.user_data.setdefault("shop_state", {"mode": "disable", "qty": 1})
    db = load_db()
    text, markup = build_main_menu(db["prices"], state["mode"], state["qty"])
    await update.message.reply_text(text, reply_markup=markup)

async def help_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("/start - Open menu\n/admin - Admin menu\n/help - Help")
...
async def on_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("paywallet:"):
        ...
        qr_file_id = db.get("payment_qr_file_ids", {}).get(wallet_key, "").strip()
        qr_file_path = db.get("payment_qr_files", {}).get(wallet_key, "").strip()
        if qr_file_id:
            try:
                await context.bot.send_document(chat_id=query.message.chat_id, document=qr_file_id, caption=qr_caption)
            except Exception as e:
                await context.bot.send_message(chat_id=query.message.chat_id, text=f"QR send failed.\nError: {e}\n\nPlease use wallet address below.")
        elif qr_file_path and os.path.exists(qr_file_path):
            try:
                with open(qr_file_path, "rb") as f:
                    await context.bot.send_document(chat_id=query.message.chat_id, document=f, caption=qr_caption)
            except Exception as e:
                await context.bot.send_message(chat_id=query.message.chat_id, text=f"QR file send failed.\nError: {e}\n\nPlease use wallet address below.")
        else:
            await context.bot.send_message(chat_id=query.message.chat_id, text="QR is not configured.\nPlease use wallet address below.")
        details_text = (
            f"💳 PAYMENT DETAILS\n\n"
            f"🔐 Type: {MODE_LABELS.get(req['mode'], req['mode'])}\n"
            f"👤 Accounts: {req['qty']}\n"
        ...
        await context.bot.send_message(chat_id=query.message.chat_id, text=f"Wallet Address:\n`{wallet['address']}`", parse_mode=ParseMode.MARKDOWN)
        await context.bot.send_message(chat_id=query.message.chat_id, text="After completing the payment, tap the button below.", reply_markup=build_i_paid_menu(req_id))
    ...
    if data.startswith("ihavepaid:"):
        ...
        save_db(db)
        await safe_edit_message(query, text="Please send your Transaction ID (TXID / Hash) now.\n\nExample:\n`a3f9c7e12b45d67890abcdef1234567890abcdef1234567890`", parse_mode=ParseMode.MARKDOWN)
        return
    ...
    if data == "admin:requests_inline":
        ...
        for req in pending[:50]:
            rows.append([InlineKeyboardButton(f"#{req['id']} | {req['name']} | {format_price(req['total_price'])} | {req['status']}", callback_data=f"admin:reqview:{req['id']}")])
    ...
    if data.startswith("admin:reqreview:"):
        ...
        save_db(db)
        await context.bot.send_message(chat_id=req["user_id"], text="🔄 Your payment is under review.\nWe will update you after checking.")
        await safe_edit_message(query, admin_request_text(req), reply_markup=request_action_menu(req_id))
        return
    ...
    if data.startswith("admin:cancelreason:"):
        ...
        save_db(db)
        await context.bot.send_message(chat_id=req["user_id"], text=f"❌ Your payment request has been cancelled.\n\nReason: {reason}\n\nPlease check payment details and submit a new request if needed.")
...
async def on_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if is_admin(user.id):
        wallet_menu_map = {
            "SET WALLET 2 (USDT TRC20)": "wallet_2",
            "SET WALLET 3 (LTC)": "wallet_3",
            "SET WALLET 4 (BNB)": "wallet_4",
            "SET WALLET 5 (USDT BEP20)": "wallet_5",
        }
        if text in wallet_menu_map:
            wallet_key = wallet_menu_map[text]
            context.user_data["await_wallet_address"] = wallet_key
            label = db["payment_wallets"][wallet_key]["label"]
            await update.message.reply_text(f"Send new address for {label}:")
            return

        if context.user_data.get("await_wallet_address"):
            wallet_key = context.user_data.pop("await_wallet_address")
            addr = text.strip()
            if len(addr) < 10:
                await update.message.reply_text("Invalid address. Send again.")
                context.user_data["await_wallet_address"] = wallet_key
                return
            db["payment_wallets"][wallet_key]["address"] = addr
            save_db(db)
            await update.message.reply_text("✅ Wallet address updated.", reply_markup=admin_reply_menu())
            return

        qr_menu_map = {
            "SET QR 2 (USDT TRC20)": "wallet_2",
            "SET QR 3 (LTC)": "wallet_3",
            "SET QR 4 (BNB)": "wallet_4",
            "SET QR 5 (USDT BEP20)": "wallet_5",
        }
        if text in qr_menu_map:
            wallet_key = qr_menu_map[text]
            context.user_data["await_qr_wallet"] = wallet_key
            label = db["payment_wallets"][wallet_key]["label"]
            await update.message.reply_text(f"Send QR now for {label}.\n\nYou can send as Document or Photo.\nRecommended: Document.")
            return

        if text == "🧾 LAST SLIP (ADMIN)":
            req = find_last_completed_request(db)
            if not req:
                await update.message.reply_text("No completed order found.", reply_markup=admin_reply_menu())
                return
            await send_slip_pdf(context, ADMIN_ID, req, f"📄 Payment slip for Request #{req['id']}.")
            await update.message.reply_text("✅ Slip sent.", reply_markup=admin_reply_menu())
            return

        if text == "🧾 LAST SLIP (USER+ADMIN)":
            req = find_last_completed_request(db)
            if not req:
                await update.message.reply_text("No completed order found.", reply_markup=admin_reply_menu())
                return
            await send_slip_pdf(context, ADMIN_ID, req, f"📄 Payment slip for Request #{req['id']}.")
            await send_slip_pdf(context, int(req["user_id"]), req, "📄 Your payment slip PDF (resend).")
            await update.message.reply_text("✅ Slip sent.", reply_markup=admin_reply_menu())
            return

        if text == "🧾 SLIP BY REQUEST ID (ADMIN)":
            context.user_data["await_slip_mode"] = "admin"
            await update.message.reply_text("Send Request ID now (example: 12):")
            return

        if text == "🧾 SLIP BY REQUEST ID (USER+ADMIN)":
            context.user_data["await_slip_mode"] = "both"
            await update.message.reply_text("Send Request ID now (example: 12):")
            return

        if context.user_data.get("await_slip_mode"):
            mode = context.user_data.pop("await_slip_mode")
            req = find_request_by_id(db, text.strip())
            if not req:
                await update.message.reply_text("Request ID not found.", reply_markup=admin_reply_menu())
                return
            await send_slip_pdf(context, ADMIN_ID, req, f"📄 Payment slip for Request #{req['id']}.")
            if mode == "both":
                await send_slip_pdf(context, int(req["user_id"]), req, "📄 Your payment slip PDF (resend).")
        ...
        if text == "☁️ Backup (Drive)":
            try:
                ...
                await update.message.reply_text(f"✅ Drive backup successful.\n\nAction: {action}\nFile ID: {file_id}", reply_markup=admin_reply_menu())
            except Exception as e:
                await update.message.reply_text(f"❌ Drive backup failed.\n\nError: {e}", reply_markup=admin_reply_menu())
            return

        if text == "⬇️ Restore (Drive)":
            try:
                drive_restore_store_db()
                await update.message.reply_text("✅ Drive restore successful.\n\nstore_db.json restored from Google Drive.", reply_markup=admin_reply_menu())
            except Exception as e:
                await update.message.reply_text(f"❌ Drive restore failed.\n\nError: {e}", reply_markup=admin_reply_menu())
            return
        ...
        if context.user_data.get("set_price_for"):
            ...
            save_db(db)
            await update.message.reply_text(f"✅ Price updated.\n\nMode: {mode}\nNew Price: {format_price(price)}", reply_markup=admin_reply_menu())
            return
    ...
    if pending_req:
        ...
        save_db(db)
        await context.bot.send_message(chat_id=ADMIN_ID, text=f"New Payment Request Received\n\n{admin_request_text(pending_req)}", reply_markup=request_action_menu(pending_req["id"]))
...
async def on_document(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not is_admin(user.id):
        return
    if context.user_data.get("await_qr_wallet"):
        wallet_key = context.user_data.pop("await_qr_wallet")
        db = load_db()
        db.setdefault("payment_qr_file_ids", {})
        doc = update.message.document
        db["payment_qr_file_ids"][wallet_key] = doc.file_id
        save_db(db)
        label = db["payment_wallets"][wallet_key]["label"]
        await update.message.reply_text(f"✅ QR saved for {label}.", reply_markup=admin_reply_menu())
        return
    ...
    if not req:
        ...
        return
    doc = update.message.document
    await context.bot.send_document(chat_id=req["user_id"], document=doc.file_id, caption="✅ Your order has been completed successfully.\n\nThank you for your patience.\nYour delivery file is attached below.")
    ...
    req["slip_pdf_path"] = pdf_file
    save_db(db)
    try:
        with open(pdf_file, "rb") as f:
            await context.bot.send_document(chat_id=req["user_id"], document=f, caption="📄 Payment Slip\n\nYour payment slip PDF is attached below.\nPlease keep it for your records.")
    except Exception as e:
        await context.bot.send_message(chat_id=ADMIN_ID, text=f"Slip PDF send to user failed for request #{req_id}.\nError: {e}")
    try:
        with open(pdf_file, "rb") as f:
            await context.bot.send_document(chat_id=ADMIN_ID, document=f, caption=f"📄 Payment slip PDF for request #{req_id}.")
    except Exception as e:
        await update.message.reply_text(f"Slip PDF admin send failed.\nError: {e}")
    context.user_data.pop("complete_request_id", None)
    await update.message.reply_text(f"✅ Order #{req_id} completed, delivered, and slip generated.", reply_markup=admin_reply_menu())
...
def main():
    ...
    if WEBHOOK_MODE:
        ...
        app.run_webhook(listen=HOST, port=PORT, webhook_url=full_webhook_url, url_path=WEBHOOK_PATH.lstrip("/"), drop_pending_updates=True)
    else:
